...
\subsection{\textcolor{\mySecondColor}{++++ Biology ++++}}

\textcolor{\mySecondColor}{++++ Brian's ++++}

We analyzed three distinct proteins.  These are the transcription factor
SH2D2A (399 amino acids in length), the alpha globin protein (142
amino acids in length) and the human SRY protein (204 amino acids in length).

For each protein, BLASTP was used to search for homologues.  The search
was restricted to refseq\_protein and to proteins from mammalian organisms.
The SH2D2A sequences were further limited to isoform X1 and the alpha
globins sequences were restricted to proteins labelled as alpha subunits.
Sequences were aligned with MUSCLE \cite{Edga:04} 
%PMID 15034147 
and any sequences with unusually long protein distances were further
eliminated by hand.

This resulted in 66 homologous SH2D2A2 sequences, 99/178 homologous
alpha globin sequences and 40 homolgous SRY sequences.  For each site in
the alignment of these proteins, the Shannon diversity and the frequency
of the most conserved amino acid was recorded and compared to the PPI
predicted by DELPHI.  Only aligned sites present in more than 10 taxa
...
%     VOLUME   = {32},
%     PAGES    = {1792-1797},
%     ABSTRACT = {We describe MUSCLE, a new computer program for creating multiple alignments of
...
%     AUADDR   = {bob@drive5.com},
%     PMID     = {15034147},
%     URL      = {https://www.ncbi.nlm.nih.gov/entrez/query.fcgi?db=PubMed&cmd=Retrieve&dopt=Citation&list_uids=15034147},
...
It facilitates use without programming skills. The user can input protein sequences and receive the prediction results via email. The PSSMs of all proteins in the testing datasets as well as the human proteome are pre-computed on the server, so they are relatively faster.
}
\begin{table}[H]
  \centering
...
\section*{Funding}
The research of L.I. is funded by a Discovery Grant (R3143A01) and a Research Tools and Instruments Grant (R3143A07) from the Natural Sciences and Engineering Research Council of Canada. \textcolor{\mySecondColor}{The research of GBG is funded by an NSERC Discovery Grant RGPIN-2020-05733.}
