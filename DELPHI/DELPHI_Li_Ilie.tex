...
DELPHI in Figure x.  There are gaps in these figures since only aligned
sites present in more than 10 taxa were included in this comparison.}

\textcolor{\mySecondColor}{In general, Figure x.a and Figure x.c shows that the locations where
...
degree of conservation in Figure x.a for the alpha haemoglobin proteins is
...
around 175aa.  Similarly for sites around 350aa and 500aa in Figure x.c
(protein SH2D2A) the PPI is low but the level of conservation is high.}

\textcolor{\mySecondColor}{The protein SRY in Figure x.b binds to sites in the DNA rather
...
\textcolor{\mySecondColor}{In general the data in Figure x show that sites with a high probability
...
\textcolor{\mySecondColor}{Caption Figure x: Three proteins were evaluated to compare the PPI
